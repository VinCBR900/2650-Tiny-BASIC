/* sim2650.c  v1.1  - Signetics 2650 Simulator
 * Target: Signetics 2650 / 2650A
 * Host:   Linux or Windows (gcc)
 *
 * Usage:  sim2650 [-t] [-b addr] image.hex
 *   -t      trace mode
 *   -b hex  breakpoint address
 *
 * v1.0 - skeleton, unverified opcodes
 * v1.1 - complete opcode table from Signetics 2650 User Manual
 *         corrected PSW layout:
 *           PSU: S(7) F(6) II(5) -(4-3) SP2(2) SP1(1) SP0(0)
 *           PSL: CC1(7) CC0(6) IDC(5) RS(4) WC(3) OVF(2) COM(1) C(0)
 *         CC in PSL bits 7-6: 00=zero 01=positive 10=negative
 *         all instruction handlers corrected to match user manual
 *         I/O modelled as stdin/stdout (SENSE=stdin bit, FLAG=stdout)
 *
 * Memory map (default):
 *   $0000-$07FF  ROM (2KB)
 *   $0800-$0FFF  RAM (2KB)
 *
 * Build: gcc -Wall -o sim2650 sim2650_v1.1.c
 */
...
int main(int argc,char *argv[]){
    ...
    if(argc<2){fprintf(stderr,"Usage: sim2650 [-t] [-b addr] image.hex\n");return 1;}
